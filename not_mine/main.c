...
#define MOUSE_RAPORT_SIZE 4
#define KEYBOARD_RAPORT_SIZE 2
#define MULTIMEDIA_RAPORT_SIZE 8
...
//#warning "a nice place for your code :)"
}
...
int SetPlaylistItemName(int item, char* itemName){
  ...
  memset(&fileListBuf[item][0],0,FILE_NAME_LEN);
  strcpy(&fileListBuf[item][0],itemName);
  return 0;
}
  ...
  if (res == FR_OK) {
      for (;;) {
          ...
          if (fno.fattrib & AM_DIR) {                    /* It is a directory */
              ...
              res = ListFilesInDirR(path);                    /* Enter the directory */
              if (res != FR_OK) break;
              path[i] = 0;
          } else {                                       /* It is a file. */
              xprintf("%s/%s\n", path, fno.fname);
  ...
  if (res == FR_OK) {
      for (;;) {
          ...
          if(strstr(fno.fname,".wav")!= NULL ){
            SetPlaylistItemName(entry, fno.fname);
            entry++;
          }
          else{
            xprintf("skipping file: %s\n",fno.fname);
          }
      }
      f_closedir(&dir);
  }
...
enum raport_mouse_type
{
  mouse_raport_buttons,
  mouse_raport_move_x,
  mouse_raport_move_y,
  mouse_raport_scroll,
};
...
enum raport_keyboard_type
{
  keyboard_raport_id,
  keyboard_raport_multimedia,
};

enum raport_multimedia_type
{
  multimedia_raport_next,
  multimedia_raport_prev,
  multimedia_raport_stop,
  multimedia_raport_eject,
  multimedia_raport_pause,
  multimedia_raport_mute,
  multimedia_raport_volume_up,
  multimedia_raport_volume_down,
};
...
int8_t modify_bit_n(int8_t num, int8_t id, int8_t value)
{
  return (num & ~(1 << id)) | (value << id);
}
...
void StartDefaultTask(void const * argument)
{
  ...
  static int is_touched = 0;
  static int8_t lastX;
  static int8_t lastY;
  static int8_t raport[MOUSE_RAPORT_SIZE] = {0};
  ...
  for(;;)
  {
    ...
    if( TS_State.touchDetected )
    {
      ...
      if(is_touched) {
        raport[mouse_raport_move_x] = cursorX - lastX;
        raport[mouse_raport_move_y] = cursorY - lastY;
      }
    ...
    USBD_HID_SendReport(&hUsbDeviceFS, raport, MOUSE_RAPORT_SIZE);
    ...
    raport[mouse_raport_move_x] = 0;
    raport[mouse_raport_move_y] = 0;
  }
  /* USER CODE END 5 */
}
